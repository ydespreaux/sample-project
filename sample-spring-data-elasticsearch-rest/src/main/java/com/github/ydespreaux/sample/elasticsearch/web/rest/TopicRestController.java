...
import com.github.ydespreaux.sample.elasticsearch.repositories.TopicRepository;
...
public class TopicRestController {
    ...
    public ResponseEntity<Topic> findTopicById(@PathVariable(name = "id") String id) {
    ...
    public ResponseEntity<List<Message>> findMessagesByQuery(@RequestParam(required = false, value = "query") String query) {
        ...
        return ResponseEntity.ok(this.repository.hasParentByQuery(queryBuilder));
    }

    @PostMapping(produces = MediaType.APPLICATION_JSON_UTF8_VALUE)
    public ResponseEntity<Topic> postMessage(@RequestBody Topic topic) {
        Topic document = this.repository.save(topic);
